...
const EasyCrop = forwardRef((props: any, ref) => {
  const {
    src,
    aspect,
    shape,
...
const ImgCrop = forwardRef((props: any, ref) => {
  const {
    aspect,
    shape,
    grid,
  ...
  const onOk = useCallback(async () => {
    ...
    canvas.toBlob(
      async (blob) => {
        ...
        //兼容IE代码
        let newFile = blobToFile(blob, 'filename_' + name)
        newFile.uid = uid
  ...
  const renderComponent = (titleOfModal: any) => (
    <>
      {renderUpload()}
      {src && (
        <Modal
